...
class TouchI2C(Touch):
    def __init__(self, i2c, cs, int_pin=None, int_handler=None,
                 width=240, height=320,
                 x_min=100, x_max=1962, y_min=100, y_max=1900):
        self.i2c = i2c
        self.i2c_addr = 0x5d # trouvé avec le scannerI2C
        super().__init__(None, cs, int_pin, int_handler, width, height, x_min, x_max, y_min, y_max)
    ...
    def send_command(self, command):
        ...
        self.cs(0)
        #self.spi.write_readinto(self.tx_buf, self.rx_buf)
        self.i2c.writeto(self.i2c_addr,self.tx_buf)
        self.cs(1)
